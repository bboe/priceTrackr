...
function track_click() {
   ...
   xmlhttp.send(null) }
if(typeof window.addEventListener != "undefined")window.addEventListener("load", track_init, false);
else if(typeof document.addEventListener != "undefined")document.addEventListener("load", track_init, false);
else if(typeof window.attachEvent != "undefined")window.attachEvent("onload", track_init);
function adsense_init() {
   if(document.all) {
      var el = document.getElementsByTagName("iframe");
      for(var i = 0; i < el.length; i++)if(el[i].src.indexOf("sample.html") > - 1)el[i].onfocus = track_click }
   else {
...
function doPageExit(e) {
   ...
   for(i = 0; i < ad.length; i++) {
      ...
      if(inFrameY && inFrameX)track_click() }
   }
...
if(typeof oldonload == "function")window.onload = function() {
   oldonload();
   AddCss();
   NiftyLoad() };
...
function AddBottom(el, side, options) {
   ...
   for(i = lim; i > 0; i--)d.appendChild(CreateStrip(i, side, color, border, btype));
...
function getElementsBySelector(selector) {
   ...
   if(selector.find("#"))if(selector.find(" ")) {
      s = selector.split(" ");
      var fs = s[0].split("#");
   ...
   for(i = 0; i < v.length; i++) {
      ...
      for(j = 0; j < c.length; j++)if(c[j] == selclass)if(tag2 == "")objlist.push(v[i]);
      else {
